...
function registerVoiceRoutes(app, { env, log }) {
  ...
  app.all('/voice/answer', (req, res) => {
    ...
    if (log) log.info('voice', 'Llamada entrante', { uuid, from });
  ...
  app.post('/voice/events', (req, res) => {
    ...
    if (log) log.info('voice', 'Evento de llamada', { uuid: uuid || null, status: status || null, duration: duration || null });
    res.sendStatus(204);
  });
}
