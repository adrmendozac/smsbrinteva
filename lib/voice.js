...
function buildNCCO(env) {
  ...
  // Only reached if the connect fails. Deliberately a talk and not a second
  // connect, so an unanswered call never opens another billed leg — and it
  // doesn't name a number, since we may have just tried to connect to it.
  ncco.push({
    action: 'talk',
    language: VOICE_LANG,
    text: 'No podemos atenderle en este momento. Por favor intente mas tarde, o responda a nuestro mensaje de texto y le contactaremos.'
  });
