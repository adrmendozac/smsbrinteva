...
async function usedToday(db, bucket, now = new Date()) {
  ...
  // Mirrors bucketFor() clause for clause. The two must agree: when sending
  // classifies a number one way and this counts it the other, the daily total
  // stops describing what was actually spent. Nameless legacy rows were the
  // case that drifted — bucketFor() reads them through KNOWN_CODES, so this
  // has to as well, or their AT&T and Verizon segments inflate the T-Mobile
  // total and pause campaigns against a budget they never spent.
  //
  // Carrier days roll at midnight UTC; UTC_DATE() keeps the boundary
  // independent of the VPS timezone.
  //
  // The leading IS NOT NULL matters beyond the sentinels: `NULL NOT IN (...)`
  // is NULL, not true, so without it a row with no code made both `isLoose`
  // and `NOT isLoose` non-true and its segments were counted in neither
  // bucket. Every row must land in exactly one.
  const looseCodes = Object.keys(KNOWN_CODES).filter(code => KNOWN_CODES[code] === OTHER);
  // bucketFor() consults KNOWN_CODES only when no name was stored; a row that
  // has a name is classified by the name alone, in both places.
  const namelessLoose = looseCodes.length
    ? ` OR (c.carrier_name IS NULL
             AND c.carrier_network_code IN (${looseCodes.map(() => '?').join(', ')}))`
    : '';
  const isLoose = `(c.carrier_network_code IS NOT NULL
        ...
        AND ((c.carrier_name IS NOT NULL AND c.carrier_name NOT REGEXP ?)${namelessLoose}))`;
  const predicate = tmobileOnly ? `NOT ${isLoose}` : isLoose;
  // MySQL REGEXP is case-insensitive for non-binary collations, matching the
  // /i on TMOBILE_NAME.
  const params = [NON_MOBILE, INVALID, TMOBILE_NAME.source, ...looseCodes];
