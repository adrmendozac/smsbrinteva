...
// Adaptive thinking lets the model reason before answering; that reasoning
// counts toward max_tokens and adds latency, so both limits leave room for it.
...
async function interpretHostedMessage(deps, rawBody) {
  ...
  // With thinking on, reasoning blocks precede the answer; read the text block.
  const rawText = response?.data?.content?.find((block) => block.type === 'text')?.text;
  let parsed;
  try {
