      ...
      "days": [
        {
          ...
          "headingLine": <int>,
          "place": "<exact substring of the heading line, or \"\" if none>",
...
Rules:
- Use "title" with origin "source" only for a genuine trip title (e.g.
  "OAXACA AL MAXIMO - 6 dias"), never for a seller's greeting to the customer.
- When an itinerary has no genuine title, suggest a concise Spanish title with
  origin "suggested" using only destination or route names that appear in the
  source. Never include a price, date, passenger count, promise, or invented
  destination. Example: "Itinerario Roma, Florencia y Venecia".
...
function validateDay(day, lines, expectedNumber) {
  ...
  if (typeof place !== 'string') return { ok: false, reason: 'invalid_schema' };
  if (place.length > 0 && !isVerbatimSubstring(place, lines[headingLine])) {
    return { ok: false, reason: 'unsafe_structure' };
  }
