...
//
// The carrier fields ride along only when the caller actually selected them.
// The alternative — always emitting them as null — would have the create and
// edit handlers, which re-read a contact without those columns, report "no
// carrier" for a contact that has one.
function shape(row) {
  const contact = {
    id: row.id,
    phone: row.phone,
  ...
  if ('carrier_network_code' in row) {
    contact.carrier_network_code = row.carrier_network_code ?? null;
    contact.carrier_name = row.carrier_name ?? null;
    contact.carrier_checked_at = row.carrier_checked_at ?? null;
  }
  return contact;
}
...
function registerContactRoutes(app, deps, requireAuth) {
  ...
  //
  // The carrier columns are here because the composer's audience is a filter
  // over this response, not a separate fetch (see the directory load in
  // App.tsx): without them every contact reaches carrierBrand() with no
  // network code and is counted "unknown", so the operator split reads zero
  // however well the backfill has run.
  app.get('/api/contacts/all', requireAuth, async (req, res) => {
    try {
      const [rows] = await db.execute(
        `SELECT id, phone, name, opted_in, archived_at,
                carrier_network_code, carrier_name, carrier_checked_at
           FROM contacts ORDER BY name IS NULL, name, id`
      );
  ...
  app.post('/api/contacts', requireAuth, validateBody(contracts.createContactRequest), async (req, res) => {
  ...
  app.patch(
    ...
    // The contract's minProperties: 1 is what used to be the handler's own
    // "Nada que actualizar" check.
    validateBody(contracts.updateContactRequest),
    async (req, res) => {
  ...
  app.patch(
    ...
    requireAuth,
    validateParams(contracts.idParams),
    // A bodyless request still means archive; the middleware supplies that
    // default so the contract can keep `archived` required for everyone who
    // does send a body.
