...
async function lookupCarrier({ axios, env }, phone) {
  // api.nexmo.com, not rest.nexmo.com: Number Insight sits on the API host
  // even though it shares api_key/api_secret auth with the Account API calls
  // above, which do live on rest. Getting this wrong 404s every request.
  const res = await axios.get('https://api.nexmo.com/ni/standard/json', {
    params: {
