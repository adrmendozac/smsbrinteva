  ...
  app.post('/status', async (req, res) => {
    ...
    if (messageId && dbStatus) {
      ...
      try {
        ...
        if (owned.length > 0) {
          await db.execute(
            `UPDATE messages SET status = ?, cost = COALESCE(?, cost) WHERE vonage_message_id = ?`,
        ...
        if (recips.length > 0) {
          const r = recips[0];
          await db.execute(`UPDATE broadcast_recipients SET status = ?, cost = COALESCE(?, cost) WHERE id = ?`, [
            dbStatus,
            cost,
            r.id
          ]);
