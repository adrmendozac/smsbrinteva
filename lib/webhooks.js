...
// state (db pool and logger) plus the things this module can't build itself:
// `sendSMS` (writes to `messages`, needs the db + Vonage creds) and the
// Kommo mirror helpers (need the KOMMO_* config assembled in index.js).
function registerWebhookRoutes(app, deps) {
  const { db, log, sendSMS, mirrorInboundToKommo, mirrorOutboundToKommo, pushKommoDeliveryStatus, sendKommoTyping, createSmsLead, createLeadNote, addLeadTags } = deps;
  ...
  app.post('/inbound', async (req, res) => {
    ...
    if (!msisdn || !text) {
      ...
      log.warn('inbound', 'Webhook entrante sin from/text — ignorado');
      return;
    }

    log.info('inbound', 'SMS entrante', {
      phone: msisdn,
    ...
    try {
      ...
      if (convRows.length === 0) {
        const [newConv] = await db.execute(
          `INSERT INTO conversations (contact_id, status) VALUES (?, 'needs_human')`,
          [contactId]
        );
      ...
      // First time this contact has texted in (no prior conversation at all):
      // create a CRM lead so a seller sees it on the board, not only in Chats.
      // Also add a note with the first message + tag as sms.
      // Best-effort — never blocks the SMS flow if Kommo is unreachable.
      if (isNewConversation && createSmsLead) {
        createSmsLead({ phone: msisdn, name: null, text }).then((leadId) => {
          if (!leadId) return;
          if (createLeadNote) {
            const preview = text.length > 200 ? text.slice(0, 200) + '…' : text;
            createLeadNote({ leadId, text: `Primer mensaje: "${preview}"` }).catch(() => {});
          }
          if (addLeadTags) {
            addLeadTags({ leadId, tags: ['sms'] }).catch(() => {});
          }
        }).catch(() => {});
      }
      ...
      // Show "typing..." in Kommo before the message arrives.
      sendKommoTyping({ phone: msisdn }).catch(() => {});
      ...
      if (OPT_OUT_KEYWORDS.has(kw)) {
        log.warn('inbound', 'Opt-out recibido', { phone: msisdn, keyword: kw });
        await db.execute(
      ...
      if (OPT_IN_KEYWORDS.has(kw)) {
        log.info('inbound', 'Opt-in recibido', { phone: msisdn, keyword: kw });
        await db.execute(
      ...
      if (HELP_KEYWORDS.has(kw)) {
        log.info('inbound', 'Help solicitado', { phone: msisdn, keyword: kw });
        const reply = registered
      ...
      log.error('inbound', 'Error procesando SMS entrante', { phone: msisdn, error: err.message });
    }
  });
  ...
  app.post('/status', async (req, res) => {
    ...
    // Messages API delivery receipts include usage.price; the legacy SMS API
    // sends message-price as a top-level field. Capture whichever is present.
    const cost = req.body.usage ? req.body.usage.price : req.body['message-price'] || null;
    console.log(`Status update for ${messageId}: ${status}${cost ? ` cost=${cost}` : ''}`);
    res.sendStatus(200);
    ...
    // Vonage puts usage.price on the *submitted* receipt, not on delivered —
    // so intermediate statuses still get a write when they carry a cost.
    // COALESCE(?, status) keeps the enum untouched for those (writing
    // 'submitted' would raise "Data truncated"), and COALESCE(?, cost) keeps
    // the submitted-time price when the delivered receipt arrives without one.
    if (messageId && (dbStatus || cost)) {
      try {
        ...
        if (owned.length > 0) {
          await db.execute(
            `UPDATE messages SET status = COALESCE(?, status), cost = COALESCE(?, cost) WHERE vonage_message_id = ?`,
            [dbStatus, cost, messageId]
          );
          // Conversational volume is low enough that every delivery is worth
          // the timeline; campaign recipients are tracked per-row in
          // broadcast_recipients and summarized at campaign end instead.
          if (dbStatus === 'delivered') {
            log.info('dlr', 'Entrega confirmada', { messageId, cost: cost ?? undefined });
          }
          if (dbStatus === 'failed') {
            log.warn('dlr', 'Entrega fallida', { messageId, status: s });
          }
          ...
          if (dbStatus && owned[0].kommo_msgid) {
            await pushKommoDeliveryStatus(owned[0].kommo_msgid, dbStatus === 'delivered' ? 1 : -1, `carrier status: ${s}`);
          }
          return;
        }
        ...
        if (recips.length > 0) {
          const r = recips[0];
          await db.execute(`UPDATE broadcast_recipients SET status = COALESCE(?, status), cost = COALESCE(?, cost) WHERE id = ?`, [
            dbStatus,
            cost,
            r.id
          ]);
          if (dbStatus === 'failed') {
            log.warn('dlr', 'Entrega fallida en campaña', { messageId, status: s });
          }
          if (dbStatus && r.kommo_msgid) {
            await pushKommoDeliveryStatus(r.kommo_msgid, dbStatus === 'delivered' ? 1 : -1, `carrier status: ${s}`);
          }
        } else {
          log.warn('dlr', 'Acuse para mensaje desconocido', { messageId, status: s });
        }
      } catch (err) {
        console.error('Status update error:', err.message);
        log.error('dlr', 'Error procesando acuse de entrega', { messageId, error: err.message });
      }
    }
  });
}
