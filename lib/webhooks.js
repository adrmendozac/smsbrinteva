...
const kommoLeads = require('./kommoLeads');
...
function registerWebhookRoutes(app, deps) {
  ...
  app.post('/inbound', async (req, res) => {
    ...
    try {
      ...
      // No open conversation for this contact: create a CRM lead so a seller
      // sees it on the board, not only in Chats, plus a first-message note and
      // an sms tag. Runs in the background so a slow Kommo never blocks the SMS
      // flow; failures are logged and queued for retry (lib/kommoLeads.js).
      if (isNewConversation && createSmsLead) {
        kommoLeads.createInboundLead(
          { db, log, createSmsLead, createLeadNote, addLeadTags },
          { phone: msisdn, text }
        );
      }
