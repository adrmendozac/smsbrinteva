const { sanitizeForSMS, SMS_TEXT_MAX, MMS_CAPTION_MAX } = require('./sms');
...
// sendEngine appends the opt-out line to every recipient's copy, so the body
// has to leave room for it or the send fails one recipient at a time, at the
// far end, after the campaign is already running.
const OPT_OUT_RESERVE = 40;
...
function registerCampaignRoutes(app, deps, requireAuth) {
  const { db, axios, env, log } = deps;

  // Opted-in contacts for the audience picker.
  app.get('/api/contacts', requireAuth, async (req, res) => {
    try {
      // All three carrier fields ride along so the composer can mirror
      // throughput.bucketFor() exactly, including sentinel, stale, and legacy
      // code-only rows, without a round trip per audience change.
      const [rows] = await db.execute(
        `SELECT id, phone, name, carrier_network_code, carrier_name, carrier_checked_at
           FROM contacts WHERE opted_in = TRUE AND archived_at IS NULL
  ...
  // One-shot Sonnet draft for the campaign body.
  app.post('/api/suggest', requireAuth, validateBody(contracts.suggestRequest), async (req, res) => {
    ...
    try {
      const ai = await axios.post('https://api.anthropic.com/v1/messages', {
        ...
        system: `You write SMS marketing copy in Spanish for Brinteva Worlds, a travel agency selling group trips to Spanish-speaking customers in the US.

OUTPUT: Reply with the SMS text only. No preamble, no quotes around it, no explanation, no alternative versions.

FORMAT:
- Plain ASCII only: no emoji, no markdown, no accent marks or tildes (write "dias" not "días", "Cancun" not "Cancún").
- Maximum 300 characters. If the idea fits in under 160, keep it under 160.

CONTENT:
- Always begin with exactly "Brinteva Worlds:" so the sender is identified.
- One offer, one call to action. The booking line is call-only and always exactly: "Llame al (925) 665-8003". Never write any other phone number, even if the request contains one.
- Warm and respectful; always address the customer as "usted", never "tu" (e.g. "Llame", "Reserve", "Aproveche", "su viaje"). No hype-caps, no exclamation chains.
- Always end with exactly "Responda STOP para cancelar".
- Use only the destinations, dates, and prices given in the request. Never invent details; if a detail is missing, write around it.`,
      ...
      const draft = ai.data.content.find(block => block.type === 'text');
      res.json({ text: sanitizeForSMS(draft ? draft.text : '') });
    } catch (err) {
  ...
  app.post('/api/campaigns', requireAuth, validateBody(contracts.createCampaignRequest), async (req, res) => {
    // Presence, types, the ISO shape of scheduledAt and the http(s) shape of
    // mediaUrl are the contract's job now; what stays here is everything that
    // depends on state or on SMS itself — the media host, the segment ceilings,
    // whether the schedule is in the past, and whether anyone is opted in.
    const { name, body, contactIds, phones, scheduledAt, mediaUrl } = req.body;
    try {
      ...
      // Without one, Vonage rejects anything over SMS_TEXT_MAX. Catch it here,
      // where the admin can still edit the text — otherwise every recipient
      // fails individually mid-send with the campaign already marked sending.
      const bodyMax = SMS_TEXT_MAX - OPT_OUT_RESERVE;
      if (!media && text.length > bodyMax) {
        return res.status(400).json({
          error: `El mensaje tiene ${text.length} caracteres; el máximo es ${bodyMax}. ` +
                 `Acórtelo o compártalo como enlace.`,
        });
      }
      ...
      log.info('campaign', `Campaña creada: ${name}`, {
        broadcastId,
      ...
      log.error('campaign', 'Error creando campaña', { error: err.message });
      res.status(500).json({ error: err.message });
    }
  });
  ...
  app.patch(
    ...
    requireAuth,
    validateParams(contracts.idParams),
    // Same default as the contact archive route: no body means archive.
    validateBody(contracts.archiveRequest, { emptyAs: { archived: true } }),
    async (req, res) => {
      try {
        ...
        log.info('campaign', archived ? 'Campaña archivada' : 'Campaña restaurada', { id: Number(req.params.id) });
      } catch (err) {
  ...
  app.get('/api/campaigns/:id', requireAuth, validateParams(contracts.idParams), async (req, res) => {
    try {
  ...
  app.post('/api/campaigns/:id/send', requireAuth, async (req, res) => {
    try {
      ...
      sendEngine.runCampaign(deps, Number(req.params.id))
        .catch(err => {
          console.error(`runCampaign ${req.params.id} failed:`, err.message);
          log.error('send', 'El motor de envío falló', { broadcastId: Number(req.params.id), error: err.message });
        });
      log.info('campaign', 'Envío disparado', { id: Number(req.params.id) });
