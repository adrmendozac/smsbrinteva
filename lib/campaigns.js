...
function registerCampaignRoutes(app, deps, requireAuth) {
  ...
  app.get('/api/contacts', requireAuth, async (req, res) => {
    try {
      // carrier_name rides along so the composer can show the audience's
      // carrier split as it is selected, without a round trip per change. It is
      // NULL until scripts/backfill-carriers.js has resolved the contact — the
      // UI counts those as "sin datos" rather than guessing at them.
      const [rows] = await db.execute(
        `SELECT id, phone, name, carrier_name
