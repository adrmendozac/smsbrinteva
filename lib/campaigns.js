...
// GSM-7 single-segment cap. Past this a plain-text send concatenates into
// multiple carrier-billed segments (153 chars each) — a 425-char body silently
// tripled the bill on a 310-recipient campaign, so a body this long over an
// audience this size costs real money before anyone notices.
...
- Use only the destinations, dates, and prices given in the request. Never invent details; if a detail is missing, write around it.`,
  ...
  app.post('/api/campaigns', requireAuth, async (req, res) => {
    ...
    try {
      ...
      // Without an image, hold every campaign to one SMS segment.
      if (!media && text.length > SMS_SEGMENT_MAX) {
        return res.status(400).json({ error: `El mensaje no puede pasar de ${SMS_SEGMENT_MAX} caracteres (1 segmento SMS). Actualmente son ${text.length}.` });
      }
