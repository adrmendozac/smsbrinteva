...
function outboundPayload({ phone, senderName, text, mediaUrl = null, msgid, botRefId, timestamp = Math.floor(Date.now() / 1000) }) {
  return {
    ...
    msgid: String(msgid),
    conversation_id: conversationRef(phone),
    sender: { ref_id: botRefId, name: senderName || 'Brinteva Worlds' },
...
// by ID instead of creating a duplicate. Resolves to { leadId, error }: the
// created lead id, or null plus a short reason. Never throws — this is a
...
async function createSmsLead({ axios, subdomain, token, pipelineId, statusId, mensajeClienteFieldId, phone, name, text }) {
  try {
    ...
    if (res.status >= 300) {
      ...
      console.error('[kommo] createSmsLead failed', res.status, detail);
      return { leadId: null, error: `HTTP ${res.status}: ${String(detail).slice(0, 300)}` };
    }
    const lead = res.data && res.data[0];
    return lead
      ? { leadId: lead.id, error: null }
      : { leadId: null, error: 'Kommo response had no lead id' };
  } catch (err) {
