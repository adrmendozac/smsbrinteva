...
function outboundPayload({ phone, senderName, text, msgid, timestamp = Math.floor(Date.now() / 1000) }) {
  return {
    timestamp,
    msec_timestamp: Date.now(),
    msgid: String(msgid),
    conversation_id: conversationRef(phone),
    sender: { id: 'brinteva-bot', name: senderName || 'Brinteva AI', ref_id: 'brinteva-bot' },
...
function registerKommoRoutes(app, deps, onAgentMessage) {
  ...
  // Raw body is required for signature verification, so bypass the global
  // JSON parser on this route only.
  app.post('/kommo/webhook/:scope_id', express.raw({ type: '*/*' }), async (req, res) => {
    ...
    try {
      ...
      console.error('[kommo] webhook JSON parse error:', err.message);
      return;
    }

    try {
      await onAgentMessage(payload, req.params.scope_id);
    } catch (err) {
