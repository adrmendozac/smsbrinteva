...
// Create a lead for a first-time inbound texter. Links to an existing Kommo
// contact by phone if one is found, otherwise creates the contact first (the
// leads endpoint only accepts linking an existing contact by id, not creating
// one inline). Returns the created lead id, or null on failure — this is a
// best-effort CRM mirror and must never be the reason /inbound fails.
async function createSmsLead({ axios, subdomain, token, pipelineId, statusId, mensajeClienteFieldId, phone, name, text }) {
  try {
    let contactId = await findContactIdByPhone({ axios, subdomain, token, phone });
    if (!contactId) {
      contactId = await createContact({ axios, subdomain, token, phone, name });
    }
    ...
    const res = await crmRequest({ axios, subdomain, token, method: 'POST', path: '/leads', data: body });
    if (res.status >= 300) {
      console.error('[kommo] createSmsLead failed', res.status, JSON.stringify(res.data));
      return null;
    }
    const lead = res.data && res.data._embedded && res.data._embedded.leads && res.data._embedded.leads[0];
