...
// "First Last" rather than however the visitor happened to type it (mobile
// keyboards auto-capitalize, so ALL CAPS and no caps both show up here).
...
function registerPublicRoutes(app, deps) {
  ...
  app.post('/api/opt-in', async (req, res) => {
    ...
    if (!String(name || '').trim() || consent !== true) {
    ...
    try {
      ...
      log.info('inbound', 'Opt-in web (formulario 10DLC)', { phone: normPhone, name: cleanName });
    } catch (err) {
...
module.exports = { registerPublicRoutes, normalizeUsPhone, toTitleCase, OPTIN_CONSENT_TEXT };
