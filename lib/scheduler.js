...
function startScheduler(deps) {
  cron.schedule('* * * * *', async () => {
    ...
    // Kept apart from the broadcast work above so a failure in one never
    // skips the other.
    try {
      await kommoLeads.retryPendingLeads(deps);
    } catch (err) {
      console.error('kommo lead retry tick error:', err.message);
      deps.log && deps.log.error('kommo', 'Error en tick de reintentos de leads', { error: err.message });
    }
  });
}
