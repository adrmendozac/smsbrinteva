...
async function runCampaign(deps, broadcastId) {
  ...
  const [bodyRows] = await db.execute(`SELECT body, media_url FROM broadcasts WHERE id = ?`, [broadcastId]);
  ...
  for (const r of recipients) {
    ...
    try {
      const { messageId } = await sendOne(deps, r.phone, body, mediaUrl);
      await db.execute(
      ...
      if (deps.mirrorCampaignToKommo && !isDryRun(env)) {
        ...
        const kommoMsgid = await deps
          .mirrorCampaignToKommo({ phone: r.phone, text: body, mediaUrl, msgid: `campaign-${broadcastId}-${r.id}` })
          .catch(e => {
