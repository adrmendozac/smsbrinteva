...
const MAX_UPLOAD = 5 * 1024 * 1024;   // what we accept from the browser
...
const MIN_EDGE = 180;                 // smallest edge carriers accept (T-Mobile)
...
async function compress(buf, format) {
  if (format === 'gif') {
    ...
    if (meta.width * h * frames > MAX_PIXELS) {
      throw Object.assign(new Error('El GIF es demasiado grande para procesar.'), { status: 413 });
    }
    if (Math.min(meta.width, h) < MIN_EDGE) {
      throw Object.assign(new Error(`La imagen es demasiado pequeña. El lado más corto debe tener al menos ${MIN_EDGE} px.`), { status: 422 });
    }
...
// Multer runs before the async route handler, so its callback errors cannot be
// caught by the handler's try/catch. Convert them to JSON here instead of
// relying on an app-wide Express error handler (index.js intentionally has
// none). `uploadMiddleware` is injected to keep this boundary independently
// testable without constructing a multipart request.
function handleUpload(uploadMiddleware, log) {
  return (req, res, next) => {
    uploadMiddleware(req, res, (err) => {
      ...
      const message = tooLarge
        ? `La imagen supera ${MAX_UPLOAD / 1024 / 1024} MB.`
        : 'No se pudo procesar la imagen subida.';

      if (log) {
        log.error('media', 'Error recibiendo imagen', {
          code: err.code || null,
          error: err.message
        });
      } else {
...
function registerMediaRoutes(app, deps, requireAuth) {
  ...
  const receiveImage = handleUpload(upload.single('image'), log);
  ...
  app.post('/api/media', requireAuth, receiveImage, async (req, res) => {
    try {
      ...
      if (log) log.info('media', 'Imagen subida', { filename, bytes: buf.length });
    } catch (err) {
      console.error({
        route: '/api/media',
      ...
      if (log) log.error('media', 'Error subiendo imagen', { filename: req.file?.originalname || null, error: err.message });
