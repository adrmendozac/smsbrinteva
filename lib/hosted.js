...
function sourceDayNumber(rawLine) {
  ...
  const value = leading?.[1] || trailing?.[1];
...
function classifyItineraries(rawBody) {
  ...
  if (markerIndexes.length > 0) {
    ...
    for (const end of [...markerIndexes, lines.length]) {
      ...
      if (!body || parseBody(body).dayCount === 0) return { kind: 'invalid-marker' };
      blocks.push(body);
      start = end + 1;
    }
    return { kind: 'explicit-appended', blocks };
  }

  let sawNumberedDay = false;
  for (const line of lines) {
    const dayNumber = sourceDayNumber(line);
    if (!dayNumber) continue;
    if (dayNumber === 1 && sawNumberedDay) return { kind: 'ambiguous' };
    sawNumberedDay = true;
  }
  return { kind: 'single', blocks: [text] };
}
...
function splitInlineWrapper(line) {
...
function parseBody(raw) {
  ...
  for (const line of lines) {
    ...
    if (firstDayFound) { currentDay.lines.push(line); continue; }

    const inline = splitInlineWrapper(line);
    if (inline) {
...
function renderHostedPage(row) {
  ...
  const blocks = classified.kind === 'explicit-appended'
    ? classified.blocks.map(parseBody)
    : [parseBody(body)];
  ...
  const bodyHtml = classified.kind === 'explicit-appended'
    ? blocks.map((block, index) => {
      const { leading, days } = renderSections(block.sections);
      const heading = escapeHtml(block.title || `Tour ${index + 1}`);
      return `<section class="itinerary-tour"><h2>${heading}</h2>${leading}${days}</section>`;
    }).join('')
    : (() => {
...
async function createHostedMessage(deps, { body, title = null, contactId = null, conversationId = null, source = 'kommo' }) {
  ...
  const parsed = parseBody(classified.blocks[0]);
