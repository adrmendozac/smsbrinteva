...
const { interpretHostedMessage, normalizeSourceLines } = require('./hostedInterpreter');
...
// Claude Sonnet 4.6 standard API list prices (USD, September 2026). The
// interpreter owns the API call; this layer only turns the returned token
// counts into operational cost metadata. Missing counts stay null rather than
// presenting a character-based guess as an actual charge.
const AI_INPUT_USD_PER_MILLION = 3;
const AI_OUTPUT_USD_PER_MILLION = 15;

function estimateAiCost(inputTokens, outputTokens) {
  ...
  return Number(((input * AI_INPUT_USD_PER_MILLION + output * AI_OUTPUT_USD_PER_MILLION) / 1_000_000).toFixed(6));
}
...
// A day token glued directly to its place with nothing but whitespace between
// them and no colon/dash separator at all right after the number — "Día 1
// Toronto", "Día 2 Toronto - Toronto" (that interior dash belongs to the
// place/route, not to a heading separator, and must not truncate it). Shared
// by matchDay() and sourceDayNumber() so the day-number extraction can never
// drift out of sync with what matchDay() actually recognizes as a day — see
// the classifyItineraries() unmarked-reset guard, which depends on exactly
// that (it runs off sourceDayNumber(), which must see every day matchDay()
// sees, or the guard silently narrows).
//
// Only the two numeric day-token shapes are covered, matching
// sourceDayNumber()'s pre-existing scope; the ordinal-word shape ("primer
// día") has never carried an extractable day number here.
const HEAD_NUMBERED_GLUED = new RegExp(
  `^(?:(?:dia|day)\\s*(\\d{1,3})` +
  `|(\\d{1,3})\\s*(?:er|ro|do|to|mo|vo|no|a|st|nd|rd|th)?\\s*(?:dia|day))\\s+(?=\\S)`
);

// A real place is short and reads as a proper noun; prose continuing the
// sentence after the day number is not ("day 1 of the conference"). This is
// the only guard standing between this shape and swallowing a whole sentence
// as a "place", so it is deliberately conservative: word-capped (mirrors
// MAX_PLACE_WORDS in hostedInterpreter.js) and capitalized, since every real
// place in these pastes is a proper noun.
const MAX_GLUED_PLACE_WORDS = 6;

// Returns { number, place } or null. NFC-normalizes and trims its own copy of
// the line rather than trusting a caller's `line`, so it is safe to call
// directly with a raw source line (sourceDayNumber() does).
function matchGluedDay(rawLine) {
  const line = String(rawLine).normalize('NFC').trim();
  const flat = deaccent(line).toLowerCase();
  const m = flat.match(HEAD_NUMBERED_GLUED);
  if (!m) return null;
  // Safe to slice `line` (not `flat`) at the flat match's length: the
  // whole-body NFC pass upstream keeps deaccenting 1-code-point-for-1 for
  // this module's input, exactly as relied on in toComparable()'s comment.
  const place = line.slice(m[0].length).trim();
  if (!place || place.length > MAX_PLACE_CHARS) return null;
  if (place.split(/\s+/).filter(Boolean).length > MAX_GLUED_PLACE_WORDS) return null;
  if (!/^\p{Lu}/u.test(place)) return null;
  if (!isConfidentPlace(place)) return null;
  return { number: Number(m[1] || m[2]), place };
}
...
function matchDay(rawLine, { allowBareNumber = false } = {}) {
  ...
  // phrase in the body. A day token immediately followed by ordinary prose
  // with no separator ("day 1 of the conference") does not qualify here: with
  // no separator the head carries the trailing prose and fails this
  // whole-shape match. That specific "glued" shape is still recognized, just
  // by matchGluedDay() below, with its own stricter guard against prose.
  ...
  // "Día 1 Toronto", "Día 2 Toronto - Toronto": the day token and its place
  // share the line with nothing but whitespace between them, no colon/dash
  // right after the number at all. Checked last and against the whole line
  // (not `head`/`place` above), because any separator elsewhere in the line —
  // like the route dash in "Toronto - Toronto" — belongs to the place, not to
  // a heading/place boundary, for this shape.
  const glued = matchGluedDay(line);
  if (glued) return { label: null, date: null, place: glued.place };
...
function sourceDayNumber(rawLine, { allowBareNumber = false } = {}) {
  ...
  const bare = allowBareNumber && matchDay(rawLine, { allowBareNumber: true })
    ...
    : null;
  // Same "glued" shape matchDay() recognizes ("Día 1 Toronto"), via the
  // identical matchGluedDay() helper matchDay() itself uses — this can never
  // see a day here that matchDay() would not also parse as one.
  const glued = !leading && !trailing && !bare ? matchGluedDay(rawLine) : null;
  const value = leading?.[1] || trailing?.[1] || bare?.[0] || (glued ? glued.number : null);
...
/* height only, width auto: these glyphs do not share one viewBox — the phone
   mark is square, share-nodes and print are not — so forcing every one into a
   fixed square stretches the wider glyphs even though the badges are the same
   size. Matching by height keeps each true to its own proportions while they
   still read as the same visual weight. */
...
.contact-button--share{width:100%;color:#fff;cursor:pointer;font:inherit;text-align:left}
.contact-button--share .contact-icon{background:#0e7490}
...
@media print{
  ...
  .contact-button--share,.contact-button--print{display:none}
...
<div class="wrap">
  ...
  <section class="contact">
    ...
    <div class="contact-actions">
      ...
      <button class="contact-button contact-button--share" type="button" data-share-itinerary>
        <span class="contact-icon"><img src="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.7.2/svgs/solid/share-nodes.svg" alt="" width="18" height="18" aria-hidden="true"></span>
        <span class="contact-copy"><span class="contact-label">Compartir</span><strong data-share-label>Compartir este itinerario</strong></span>
      </button>
...
function decodeAiStructure(value) {
...
function sourceRange(lines, start, end) {
...
// Convert the source-index references validated by hostedInterpreter into the
// parser's existing render shape. Customer-visible body text is always sliced
// from the original normalized source lines; the stored JSON cannot inject
// replacement prose into the page.
function aiStructureToBlocks(body, rawStructure) {
  const structure = decodeAiStructure(rawStructure);
  if (!structure || !Array.isArray(structure.tours) || structure.tours.length === 0) return null;
  if (!['itinerary', 'multiple_itineraries', 'travel_offers'].includes(structure.classification)) return null;

  const lines = normalizeSourceLines(body);
  if (lines.length === 0) return null;
  let preambleLines = [];
  if (structure.preamble) {
    preambleLines = sourceRange(lines, structure.preamble.startLine, structure.preamble.endLine);
    if (!preambleLines) return null;
  }

  const blocks = [];
  for (let tourIndex = 0; tourIndex < structure.tours.length; tourIndex++) {
    const tour = structure.tours[tourIndex];
    if (!tour || !Array.isArray(tour.days)) return null;
    const titleLine = tour.titleLine;
    if (titleLine !== null && (!Number.isInteger(titleLine) || titleLine < 0 || titleLine >= lines.length)) return null;

    const sections = [];
    if (tourIndex === 0 && preambleLines.length) {
      sections.push({ type: 'text', lines: preambleLines });
    }

    for (const day of tour.days) {
      if (!day || !Number.isInteger(day.number) || !Number.isInteger(day.headingLine)) return null;
      if (day.headingLine < 0 || day.headingLine >= lines.length) return null;
      const content = sourceRange(lines, day.contentStartLine, day.contentEndLine);
      if (!content || day.contentStartLine <= day.headingLine) return null;
      const heading = lines[day.headingLine];
      const place = typeof day.place === 'string' && (!day.place || heading.includes(day.place)) ? day.place : '';
      sections.push({
        type: 'day',
        label: `Día ${day.number}`,
        date: null,
        place,
        lines: content,
      });
    ...
    // Phase 1 intentionally stores day-less travel offers without their
    // contentEndLine. Their next title (or EOF) is an unambiguous boundary, so
    // reconstruct the body range from the validated title line indexes.
    if (tour.days.length === 0) {
      if (titleLine === null) return null;
      const nextTitle = structure.tours[tourIndex + 1]?.titleLine;
      const end = Number.isInteger(nextTitle) ? nextTitle - 1 : lines.length - 1;
      const content = titleLine + 1 <= end ? sourceRange(lines, titleLine + 1, end) : [];
      if (content === null) return null;
      if (content.length) sections.push({ type: 'text', lines: content });
    }

    const suggestedTitle = structure.title && structure.title.origin === 'suggested'
      ? String(structure.title.value || '').trim()
      : '';
    const blockTitle = titleLine !== null
      ? lines[titleLine]
      : (tourIndex === 0 ? suggestedTitle : '');
    blocks.push({
      title: blockTitle || deriveTitle(sections),
      preamble: null,
      sections,
  ...
  return { classification: structure.classification, blocks };
}

function deterministicBlocks(body) {
  ...
  const blocks = classified.kind === 'explicit-appended'
    ...
    : [parseBody(body, opts)];
  return { classification: classified.kind, blocks };
}
...
// absent or invalid ones simply render no hero. AI is never called here: only
// the validated structure saved at creation time is read.
function renderHostedPage(row) {
  const { title, body } = row || {};
  const ai = aiStructureToBlocks(body, row && row.ai_structure);
  const resolved = ai || deterministicBlocks(body);
  ...
  const multipleBlocks = blocks.length > 1 || resolved.classification === 'travel_offers';
  const bodyHtml = multipleBlocks
  ...
  const metaLine = dayCount > 0
    ...
    : '';
  return page({
    title: title || parsed.title || 'Itinerario de viaje',
    bodyHtml,
    metaLine,
    hasExpandableDays,
...
function storedAiStructure(result) {
...
async function analyzeHostedBody(deps, text, explicitTitle = null) {
  const { env = {}, axios, log } = deps;
  let interpretation = null;
  let aiStructure = null;
  let aiResolved = null;

  if (env.ANTHROPIC_API_KEY && axios && typeof axios.post === 'function') {
    let responseUsage = null;
    const meteredAxios = {
      post: async (...args) => {
        const response = await axios.post(...args);
        responseUsage = response?.data?.usage || null;
        return response;
      },
    };
    interpretation = await interpretHostedMessage({ ...deps, axios: meteredAxios }, text);
    if (interpretation.ok && responseUsage) {
    ...
    if (interpretation.ok) {
      aiStructure = storedAiStructure(interpretation);
      aiResolved = aiStructureToBlocks(text, aiStructure);
      if (!aiResolved && interpretation.classification !== 'general_text') {
        log?.warn?.('hosted', 'Estructura de Haiku no pudo convertirse para renderizar', {
  ...
  let resolved = aiResolved;
  let parseMethod = aiResolved ? 'haiku' : 'deterministic';
  if (!resolved) {
    ...
    resolved = deterministicBlocks(text);
  }

  const parsed = resolved.blocks[0];
  const aiTitle = aiResolved && interpretation?.title
    ? String(interpretation.title.value || '').trim()
    : '';
  let resolvedTitle;
  let titleOrigin;
  if (explicitTitle != null && String(explicitTitle).trim()) {
    resolvedTitle = String(explicitTitle).trim();
    titleOrigin = 'provided';
  } else if (aiTitle) {
    resolvedTitle = aiTitle;
    titleOrigin = interpretation.title.origin === 'suggested' ? 'suggested' : 'source';
  } else if (aiResolved) {
    resolvedTitle = interpretation.classification === 'travel_offers'
      ? 'Opciones de viaje'
      : interpretation.classification === 'multiple_itineraries'
        ? 'Itinerarios de viaje'
        : 'Itinerario de viaje';
    titleOrigin = 'fallback';
  } else {
    resolvedTitle = String(parsed.title || '').trim();
    titleOrigin = resolvedTitle ? 'deterministic' : 'fallback';
    if (!resolvedTitle) resolvedTitle = 'Itinerario de viaje';
  }
  resolvedTitle = resolvedTitle.slice(0, 200);

  const usage = interpretation?.usage || {};
  const inputTokens = usage.inputTokens != null && Number.isFinite(Number(usage.inputTokens))
    ? Number(usage.inputTokens) : null;
  const outputTokens = usage.outputTokens != null && Number.isFinite(Number(usage.outputTokens))
    ? Number(usage.outputTokens) : null;
  const estimatedCostUsd = inputTokens === null || outputTokens === null
    ? null
    : estimateAiCost(inputTokens, outputTokens);

  log?.info?.('hosted', 'Mensaje alojado analizado', {
    parseMethod,
    classification: aiResolved ? interpretation.classification : resolved.classification,
    fallbackReason: interpretation && !interpretation.ok ? interpretation.reason : null,
    model: usage.model || null,
    durationMs: Number.isFinite(Number(usage.durationMs)) ? Number(usage.durationMs) : null,
    inputTokens,
    outputTokens,
    estimatedCostUsd,
    titleOrigin,
  ...
  return {
    aiStructure,
    parseMethod,
    parseModel: usage.model || null,
    parseDurationMs: Number.isFinite(Number(usage.durationMs)) ? Number(usage.durationMs) : null,
    inputTokens,
    outputTokens,
    estimatedCostUsd,
    titleOrigin,
...
async function createHostedMessage(deps, { body, title = null, contactId = null, conversationId = null, source = 'kommo' }) {
  ...
  if (!text.trim()) throw new Error('El mensaje a alojar está vacío');
  const bytes = bodyByteLength(text);
  if (bytes > MAX_BODY_BYTES) {
    throw new Error(`El mensaje ocupa ${bytes} bytes; el máximo es ${MAX_BODY_BYTES}`);
  }

  const analysis = await analyzeHostedBody(deps, text, title);
  const { parsed, resolvedTitle } = analysis;
      ...
      const [ins] = await db.execute(
        `INSERT INTO hosted_messages
           (code, title, body, contact_id, conversation_id, source, expires_at,
            ai_structure, parse_method, parse_model, parse_duration_ms,
            parse_input_tokens, parse_output_tokens, parse_cost_usd,
            title_origin, parsed_at,
  ...
  app.get('/i/:code', async (req, res) => {
    ...
    try {
      ...
      const [rows] = await db.execute(
        `SELECT id, title, body, ai_structure, parse_method, title_origin,
                hero_destination, hero_photo_id, hero_image_url, hero_photo_url,
                hero_photographer_name, hero_photographer_url,
