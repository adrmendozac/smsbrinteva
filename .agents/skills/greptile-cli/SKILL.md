...
the interactive confirmation, so surface that step for the user. Prefer `greptile init --json`: it
never prompts and prints one object to stdout whose `status` field tells the exit-`1` outcomes
apart: `enabled` (exit `0`), `not_enabled` (ask the user to run `greptile init` in a terminal),
`not_connected` (the repository is not connected to the user's Greptile workspace), or
`unsupported_server` (the server predates `init`).
